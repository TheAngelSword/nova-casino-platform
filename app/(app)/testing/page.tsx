import { Bug, ClipboardCheck, Clock, ShieldAlert, UserCheck } from 'lucide-react';
  ...
  return (
    <div className="space-y-7">
      <section>
        ...
        <p className="nova-subtitle">Registro de testers, horarios laborados, alcance revisado, casos de prueba, bugs, evidencias y revisión de responsables.</p>
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        ...
        <div className="nova-card-pad">
          ...
          <form action={createBugReportAction} className="mt-4 grid gap-3 md:grid-cols-2">
            ...
            <select name="severity" className="nova-select"><option value="bloqueante">Bloqueante</option><option value="critica">Crítica</option><option value="alta">Alta</option><option value="media">Media</option><option value="baja">Baja</option></select>
            ...
            <select name="assigned_area" className="nova-select"><option>Programación</option><option>Diseño</option><option>Arte</option><option>Audio</option><option>Matemática</option><option>Integración</option><option>Homologación</option></select>
            <input name="assigned_to_name" className="nova-input" placeholder="Responsable" />
            <button className="nova-btn-primary md:col-span-2" type="submit">Guardar bug</button>
          </form>
        </div>
      </section>
        ...
        <div className="nova-card-pad overflow-hidden">
          ...
          <div className="mt-4 overflow-x-auto">
            <table className="w-full min-w-[820px] text-left text-sm">
              <thead className="border-b border-white/10 text-xs uppercase tracking-wider text-white/45"><tr><th className="py-3">Bug</th><th>Juego</th><th>Severidad</th><th>Estado</th><th>Área</th><th>Responsable</th></tr></thead>
              <tbody className="divide-y divide-white/10">
                {bugs.map((b: any) => (
                  <tr key={b.id} className="text-nova-text2"><td className="py-3 font-semibold text-white">{b.title}</td><td>{b.games?.name ?? '—'}</td><td><TestingStatusPill value={b.severity} /></td><td><TestingStatusPill value={b.status} /></td><td>{b.assigned_area ?? '—'}</td><td>{b.assigned_to_name ?? '—'}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
        ...
        <aside className="space-y-5">
          <div className="nova-card-pad">
            ...
            <div className="mt-4 space-y-3">
              {sessions.map((s: any) => <div key={s.id} className="rounded-xl border border-white/10 bg-white/[0.035] p-3"><div className="text-sm font-semibold text-white">{s.games?.name ?? 'Juego'} · {s.build_version ?? 'Build'}</div><div className="text-xs text-nova-text2">{s.session_date} · {s.hours_worked ?? 0} h · {s.test_scope}</div></div>)}
            </div>
          </div>
          <div className="nova-card-pad">
            <h2 className="font-display text-lg font-bold">Casos de prueba</h2>
            <div className="mt-4 space-y-3">
              {cases.slice(0, 8).map((c: any) => <div key={c.id} className="rounded-xl border border-white/10 bg-white/[0.035] p-3"><div className="text-sm font-semibold text-white">{c.title}</div><div className="text-xs text-nova-text2">{c.games?.name ?? 'General'} · {c.area} · {c.priority}</div></div>)}
            </div>
          </div>
        </aside>
      </section>
